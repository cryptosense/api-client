...
let test_request
  ...
  , fun () ->
      ...
      check
        ...
        (name ^ " headers") expected_headers header;
      check (list (pair string string)) (name ^ " form") expected_form form;
      check
        (option (pair string int))
        (name ^ " file") expected_file (file_to_tuple file) )

let request_builder_tests =
  let api = Api.make ~api_endpoint:"endpoint" ~api_key:"KEY" in
  [ test_request ~name:"S3 Signature request"
      ~expected_url:"endpoint/api/v1/trace_s3_post" ~expected_method:"POST"
      ~expected_headers:[("API-KEY", "KEY")] ~expected_form:[]
      ~expected_file:None
      (Cs_api_core.build_s3_signed_post_request ~api)
  ; test_request ~name:"File upload request" ~expected_url:"url"
      ~expected_method:"POST" ~expected_headers:[]
      ~expected_form:
        [ ("key", "abc")
        ; ("signature", "cde")
        ; ("Content-Type", "")
  ...
  ; test_request ~name:"Trace import request"
      ~expected_url:"endpoint/api/v1/projects/9/traces" ~expected_method:"POST"
      ~expected_headers:[("API-KEY", "KEY")]
      ~expected_form:[("key", "abc"); ("name", "cde"); ("size", "10")]
      ~expected_file:None
