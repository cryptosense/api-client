...
let multipart_request_to_string mp =
  let body_result = mp |> Multipart_form_writer.r_body |> Lwt_main.run in
  let header_result = mp |> Multipart_form_writer.r_headers |> Lwt_main.run in
  match (header_result, body_result) with
  | (Ok headers, Ok stream) ->
    (headers, stream |> Lwt_stream.get_available |> String.concat "")
  | (_, Error err)
  | (Error err, _) ->
    ([], err)
