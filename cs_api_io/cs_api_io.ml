...
let multipart_file_upload_body form_data file_path file_size =
  ...
  let file_stream = file_content |> Lwt_io.read_chars |> Lwt_stream.map (fun (c) -> String.make 1 c) in
  ...
  let form_parts_size = 
    ...
    |> List.fold_left (+) 0 in
  let file_header = "Content-Disposition: form-data; name=\"file\"; filename=\"" ^ file_path ^ "\"\r\nContent-Type: application/octet-stream\r\n\r\n" in
  let file_header_size = String.length file_header in
  let final_header =  "\r\n--" ^ sep ^ "--\r\n" in
  let final_header_size = String.length final_header in
  let part1 = [ initial_header ]
              @ form_parts
              @ [ file_header ] in
  let part1_stream = Lwt_stream.of_list part1 in
  let part2_stream = Lwt_stream.of_list [ final_header ] in
  let entire_stream = Lwt_stream.of_list [ part1_stream
                                         ; file_stream
                                         ; part2_stream 
                                         ]  in
  let entire_size = initial_header_size + form_parts_size + file_header_size + final_header_size + file_size in
