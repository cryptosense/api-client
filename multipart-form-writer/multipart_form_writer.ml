...
let init_with_separator separator = {MultipartRequest.elements = []; separator}

let add_form_element ~name ~value mp =
  let open MultipartRequest in
  {mp with elements = Form {key = name; value} :: mp.elements}

let add_file_from_disk ~name ~path mp =
  let open MultipartRequest in
  {mp with elements = File {path; name} :: mp.elements}

let file_size path =
  path |> Lwt_io.file_length |> Lwt.map Int64.to_int |> Lwt_result.ok
...
let safe_file_size path =
  ...
  | Unix.Unix_error (Unix.EISDIR, _, _) ->
    Lwt_result.fail ("File " ^ path ^ " is a directory")
...
let open_file path =
  ...
  let read_while_not_empty channel () =
    Lwt_io.read ~count:4096 channel >|= fun chunck ->
    match chunck with
    | "" -> None
    | _ -> Some chunck
  in
  ...
  >|= Lwt_stream.from
  >>= fun file_stream ->
  safe_file_size path >|= fun file_size ->
...
let element_to_string separator element =
  ...
  | File {path; _} ->
    ...
    safe_open_file path >|= fun file_stream ->
    Lwt_stream.append file_header_stream file_stream

let element_size separator element =
  match element with
  | MultipartRequest.Form _ ->
    Lwt_result.return (String.length (element_separator_line separator element))
  | File {path; _} ->
    let open Lwt_result.Infix in
    let file_header = element_separator_line separator element in
    let file_header_size = String.length file_header in
    safe_file_size path >|= fun file_size -> file_header_size + file_size

let rec mfoldl f acc l =
  match l with
  | h :: t -> Lwt_result.bind h (fun value -> mfoldl f (f value acc) t)
